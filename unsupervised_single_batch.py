...
class graphSAGESSL(nn.Module):
    def __init__(self,edge_types,hidden_dim,output_dim):
        ...
        self.conv2 = HeteroConv({edge_type : SAGEConv((-1,-1),hidden_dim) for edge_type in edge_types},aggr='sum')
    ...
    def forward(self,x_dict,edge_index_dict):
        ...
        return x_dict,self.head(x_dict['paper'])
    

hidden_dim = batch['paper'].x.shape[1]
out_channels = max(batch['paper'].y).item() + 1

model = graphSAGESSL(batch.edge_types,hidden_dim,out_channels)
...
def mask_x_dict(x_dict,p=0.75):
    ...
    for k,v in x_dict.items():
        ...
        x_dict_masked[k] = x_dict[k].clone()*(mask[k].unsqueeze(-1)) + unk_emb[k](torch.zeros_like(mask[k],dtype=torch.long))*(~mask[k].unsqueeze(-1))
